...
places = ['Красная пл-дь, 1', 'Саратов, пр-т Строителей, 36', 'Саратов, Б.Горная, 1']
for place in places:
    ...
    if response:
        ...
        print(result)
        address = result['metaDataProperty']['GeocoderMetaData']['Address']
        try:
            index = address['postal_code']
        except Exception:
            index= ''
        full_address = f"{address['formatted']} ({index})"
